...
差别在于：前者传入期约的时候，会不做任何改变(幂等操作)，而后者传入期约，会作为错误的理由

Thenable接口：异步结构中，都实现了then()方法，接收2个函数，分别是成功函数和失败函数

p.then()的返回值：
使用Promise.resolve()来包装onResolved程序的返回值，无返回值，则包装undefined；如果连onResolved程序都没有，则包装上一个期约解决的值。（此种情况是，Promise为pending或者是resolved状态，然后p.then()的返回值，在同步执行下是promise-pending，在异步执行下，才会遵循上述规则）

关于then()、catch()还有finally()的返回值，看书P330
